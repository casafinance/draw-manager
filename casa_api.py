...
class CasaApi:
    ...
    def _pull(self, selected_tabs):
        ...
        if have_sheets:
            ...
            use_api = bool(L._service_account_path() or
                           (L.google_creds_path().exists() and
                            L.google_token_path().exists()))
            if use_api:
                try:
        ...
        for p in all_properties:
            key = (L.addr_norm(p["address"]), L.parse_amount(p.get("amount")))
            ex = existing_by_key.get(key)
            if ex and L.is_row_complete(ex):
                # Skip from FCI but still display the saved values in the table.
                p.setdefault("loan_number", ex.get("loan_number"))
                p.setdefault("account_number", ex.get("account_number"))
                p.setdefault("cf_number", ex.get("cf_number"))
                p.setdefault("date_received", ex.get("date_received"))
                p["_status"] = "skip"
            else:
                if ex:
                    p.setdefault("loan_number", ex.get("loan_number"))
    ...
    def _fci(self):
        ...
        overwrite = bool(self.cfg.get("fci_overwrite", False))
        ...
        # With overwrite=True, re-look up everything (even rows that already have numbers).
        to_lookup = []
        already_ok = 0
        skipped = 0
        for p in self.properties:
            ...
            if has_loan and has_acct and not overwrite:
                already_ok += 1
                continue
            to_lookup.append(p)

        overwrite_note = " (overwrite mode — re-looking up all)" if overwrite else ""

        self.log(f"FCI: {len(self.properties)} properties — "
                 f"{skipped} already in file, {already_ok} already have both "
                 f"numbers, {len(to_lookup)} to look up{overwrite_note}.")
        if not to_lookup:
            self.log("Nothing needs an FCI lookup. Enable 'Overwrite previous "
                     "Excel values' in settings to force a re-lookup.")
    ...
    def _save(self):
        ...
        if not to_write:
            ...
            return
        self.status("Saving…")
        files = self._save_outputs(out, to_write, overwrite=overwrite)
    ...
    def _save_outputs(self, master_path, rows, overwrite=False):
        def strip(r):
            return {k: v for k, v in r.items() if k != "_date_tag"}
        written = [master_path]
        self.log(f"[master] {os.path.basename(master_path)}")
        self._merge_and_save(master_path, [strip(r) for r in rows], overwrite=overwrite)
        ...
        for tag, grp in sorted(groups.items()):
            ...
            self._merge_and_save(per_path, grp, overwrite=overwrite)
            written.append(per_path)
        return written

    def _merge_and_save(self, path, new_rows, overwrite=False):
            ...
            if key in merged_by_key:
                ...
                for k, v in nr.items():
                    if overwrite and v is not None:
                        target[k] = v        # overwrite mode: replace everything
                    elif not target.get(k) and v:
                        target[k] = v        # normal mode: only fill blanks
                updated += 1
            else:
                merged.append(nr)
