...
def get_google_creds(interactive=True):
    """Return authenticated Google credentials, doing the OAuth flow on first
    use. Requires `credentials.json` (OAuth client downloaded from Google
    Cloud Console) to sit next to the app. Saves a refresh token to
    ...
    if not interactive:
        raise RuntimeError("Google token missing/expired and interactive=False.")

    cp = google_creds_path()
    if not cp.exists():
        raise RuntimeError(
            f"credentials.json not found in {APP_DIR}.\n\n"
            f"One-time setup (about 10 min):\n"
            f"  1. Go to https://console.cloud.google.com/\n"
            f"  2. Create a new project (name doesn't matter)\n"
            f"  3. APIs & Services → Library → search 'Sheets API' → Enable\n"
            f"  4. APIs & Services → OAuth consent screen → External → fill required\n"
            f"     fields → add your Google account as a Test user\n"
            f"  5. APIs & Services → Credentials → Create Credentials → OAuth\n"
            f"     client ID → Application type: Desktop app → Create\n"
            f"  6. Download the JSON, rename to credentials.json, place it at:\n"
            f"     {cp}\n"
            f"  7. Click 'Sign in to Google' in the app — a browser tab will\n"
            f"     open for you to grant access. Token is saved locally."
        )
